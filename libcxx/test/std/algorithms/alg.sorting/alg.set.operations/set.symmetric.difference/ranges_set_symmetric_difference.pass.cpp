...
constexpr void
testSetSymmetricDifferenceImpl(TestArray<int, N1> in1, TestArray<int, N2> in2, TestArray<int, N3> expected) {
...
constexpr void testImpl() {
  // range 1 shorter than range2
  {
    ...
    TestArray expected{0, 1, 3, 5, 7, 10, 13, 15, 100};
  ...
  // range 2 shorter than range 1
  {
    ...
    TestArray in2{0, 2, 8};
    TestArray expected{0, 6, 12, 15, 16};
    testSetSymmetricDifferenceImpl<In1, In2, Out>(in1, in2, expected);
  }
  ...
  // range 1 and range 2 has the same length but different elements
  {
    ...
    TestArray expected{0, 6, 12, 16, 17, 19};
    testSetSymmetricDifferenceImpl<In1, In2, Out>(in1, in2, expected);
  }
  ...
  // check that ranges::dangling is returned for non-borrowed_range
  {
    ...
    assert(base(result.out) == out.data() + out.size());
    assert(std::ranges::equal(out, TestArray{2, 4, 5, 7, 9}));
  }
}
...
constexpr bool test() {
  // check that every element is copied exactly once
  {
    ...
    // iterator overload
    {
      ...
      assert(std::ranges::equal(out, TestArray<TracedCopy, 4>{1, 5, 15, 16}));
  ...
  // Test custom comparator
  {
    ...
    // iterator overload
    {
      ...
      assert(std::ranges::equal(out, TestArray{4, 9, 12}, {}, &Data::data));
  ...
  // Test Projection
  {
    ...
    // iterator overload
    {
      ...
      assert(std::ranges::equal(out, TestArray{1, 2}, {}, &Data::data));
  ...
  // Complexity: At most 2 * ((last1 - first1) + (last2 - first2)) - 1 comparisons and applications of each projection.
  {
    ...
    TestArray<Data, 1> r1{{{7}}};
    TestArray<Data, 6> r2{{{1}, {2}, {3}, {4}, {5}, {6}}};
    TestArray expected{1, 2, 3, 4, 5, 6, 7};

    const std::size_t maxOperation = 2 * (r1.size() + r2.size()) - 1;

    // iterator overload
    {
      TestArray<Data, 7> out;
      CompProjs compProjs{};
