...
template <class S>
consteval void test_appending(size_t k, size_t N, size_t new_capacity) {
  assert(N > k);
...
template <class S>
consteval void test_truncating(size_t o, size_t N) {
  assert(N < o);
  auto s = S(o, 'a');
...
consteval void test_value_categories() {
  std::string s;
